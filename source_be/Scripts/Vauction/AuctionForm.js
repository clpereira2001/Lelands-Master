function InitAuctionForm(data) {
    ...
    if (data == null) {
        ...
        $("#form_auction_LOA, #form_auction_IsPulledOut, #form_auction_IsUnsold,#form_auction_IsCatalog, #form_auction_IsPrinted, #form_auction_Photographed, #form_auction_InLayout").removeAttr("checked");
        ...
        $("#form_auction_IsCatalog").attr("checked", data.catl);
...
function SubmitAuctionForm() {
    ...
    var tblSummaryObj = $('img_list');
    var rowCount = $('#img_list tr').length;
   
    $('#img_list tr').each(function (i, el) {

        var $tds = $(this).find('td'),
            productId = $tds.eq(0).text(),
            product = $tds.eq(5).children().text();
            //Quantity = $tds.eq(2).text();
       
        $chk = $(this).find(':checkbox:checked').val();
        var imgDesc;
        if($chk=="on")
        {
            imgDesc = "1|" + productId;
            imageTags.push(imgDesc);
        }
        else
        {
            imgDesc = "0|" + productId;
    ...
    return {
        ...
        IsCatalog: $("#form_auction_IsCatalog").attr("checked"),
